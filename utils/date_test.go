...
func Test_GetDateRangeBounds(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(got.Format(got2), tt.want) && !tt.wantErr {
				t.Errorf("GetDateRangeBounds() got = %v, want %v", got.Format(got2), tt.want)
				return
			}
			if !reflect.DeepEqual(got1.Format(got2), tt.want1) && !tt.wantErr {
				t.Errorf("GetDateRangeBounds() got1 = %v, want %v", got1.Format(got2), tt.want1)
				return
			}
